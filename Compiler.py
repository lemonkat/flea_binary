...
class Line:
	...
	def add_aligns(self, left:int, right:int) -> "Line":
		return Line(powerline = self.powerline, align_left = self.align_left, main = (self.align_left * left) + self.main + (self.align_right * right), align_right = self.align_right)

	def __len__(self) -> int:
		return 2 + len(self.main)

	def __str__(self) -> str:
		return self.powerline + "|" + self.align_left + "|" + self.main + "|" + self.align_right
...
class Blueprint:
	...
	def add_blueprint(cls, obj, blueprint:"Blueprint", shift:int = 0) -> None:
		...
		for line in blueprint.lines[::-1]:
			if blueprint.name == "GAP":
				Blueprint.add_line(obj, line.add_aligns(shift, obj.num_wires() - shift - blueprint.num_inputs))
			else:
				Blueprint.add_line(obj, line.add_aligns(shift, obj.num_wires() - shift - blueprint.num_inputs + correction))
	...
	def from_str(cls, s:str) -> "Blueprint":
		...
		for line in lines[:0:-1]:
			line = Line(powerline = line[0:2], align_left = line[2:4], main = line[4:-2], align_right = line[-2:])
...
#class BlueprintLvl2(Blueprint):
	...
	def move(cls, obj:"Blueprint", wire_name:str, position:int) -> None:
		...
		if current_pos == position:
			...
			Blueprint.add_blueprint(obj, Blueprint.library["SWAP"], shift = current_pos - 1)
			obj.wires[current_pos], obj.wires[current_pos - 1] = obj.wires[current_pos - 1], obj.wires[current_pos]
		else:
	...
	@classmethod
	def call(cls, obj:"Blueprint", inputs:list[str], outputs:list[str], gate:"Blueprint") -> None:
	...
	def delete(cls, obj:"Blueprint", wire_name:str) -> None:
		if wire_name in obj.wires:
			Blueprint.add_blueprint(obj, Blueprint.library["DEL"], shift = obj.wires.index(wire_name))
	...
	@classmethod
	def if_statement(cls, obj:"Blueprint", condition:str, inputs:list[str], outputs:list[str], gate:"Blueprint") -> None:
		Blueprint.call(obj, [condition], ["SYS_IF"], Blueprint.library["COPY"])
		Blueprint.call(obj, [wire + "_SYS_BRANCH" for wire in inputs], [wire + "_SYSBRANCH" for wire in outputs], gate)
		for wire in outputs:
			if wire in obj.wires:
				Blueprint.call(obj, ["SYS_IF", wire, wire + "_SYS_BRANCH"], [wire], Blueprint.library["IF"])
				Blueprint.delete_wire(obj, "SYS_IF")
			else:
				Blueprint.call(obj, ["SYS_IF", wire + "_SYS_BRANCH"], [wire], Blueprint.library["AND"])
...
class Compiler:
				...
				if line[0] == "IMPORT":
					...
					while line[0] != "END":
						...
						if line[0] == "ORDER":
							...
							cmds.append((Blueprint.call, [line[to_pos + 2:], line[1:to_pos], Blueprint.library[line[to_pos + 1]]]))
						elif line[0] == "IF":
							to_pos = line.index("TO")
							cmds.append((Blueprint.if_statement, [line[to_pos + 2:], line[2:to_pos], Blueprint.library[line[to_pos + 1]]]))
						elif line[0] == "DEL":
							cmds.append((Blueprint.delete_wires, [line[1:]]))
						elif line[0] == "GAP":
							cmds.append((Blueprint.call, [[], [], Blueprint.library["GAP"]]))
						elif line[0] == "CLEAR":
	...
	@classmethod
	def run(cls, args:list[bool], cell_size: int, color_profile:dict, flea_color:str, wait:float = 0, jump:int = 10000) -> list[bool]:
		...
		return [board.is_flea_at(pos, 0) for pos in range(2, (main.num_wires() * 2 + 2), 2)]
	...
	def get_board(cls, cell_size, color_profile, y_mult = 1) -> Board:
		...
		board.board = [[*(line.powerline + line.main + " " * (width - len(line)))] for line in main.lines]
		board.reload()
		board.update()
